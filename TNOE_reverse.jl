...
# based on v3, has development at the end of the lifespan rather than the beginning 
...
for cred in CredLimits #cycles through specified infections chances, conducts (Runs) simulations at a given level of infection 
    for batch in 1
        ...
        while Run <= Runs #conducts Runs number of simulations
            ...
            print('\n',"******** TNOE " ,Timing," Credit limit ",Credits, " Fitness Effects ",OrgFitBalance," Batch ",batch," ********** ", (Dates.Time(Dates.now()) - Start_Time)/Nanosecond(1)* (1/1000000000))
        ...
        FileName = string(workDir,"/Data/Batches/",string(inf*100)[1:end-2],"_PercentInf_EC_Limit_",string(Credits*100)[1:end-2],"_FitEff_",OrgFitBalance,"_TNOE_",Timing,"_Batch_",batch,"_reverse.jld2")
