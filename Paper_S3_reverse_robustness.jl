# v2 changes
#  swapped plots to show the asynch and synch on the same plots at a single fitness balance
using Statistics
using FileIO
...
cd(Data)
TNTEFiles = filter(x->endswith(x, ".jld2"), readdir())
TNTEFiles = filter(x -> contains(x, "TNTE"), TNTEFiles)
TNTEFiles = filter(x -> contains(x, "reverse"), TNTEFiles)
...
TNOEFiles = filter(x->endswith(x, ".jld2"), readdir())
TNOEFiles = filter(x -> contains(x, "TNOE"), TNOEFiles)
TNOEFiles = filter(x -> contains(x, "reverse"), TNOEFiles)
...
#region Plot robustness for the balanced fitness case comparing synchronous and asycnhronous 
...
save("S3_reverse_robustness.pdf", f)
#endregion