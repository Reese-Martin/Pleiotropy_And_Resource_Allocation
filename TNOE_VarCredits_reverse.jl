# based on v2, has development at the end of the lifespan rather than the beginning 
...
function Simulation(NumHosts,NumPars,Generations,OrgFitBalance,Timing,Start_Time,CredSwitch,Run)# function that executes a single simulation consisting of "Generations" number of generations 
    ...
    for i in 1:NumHosts
        ...
        tmp[1:4,5:8] .= 0
        tmp[5:8,1:4] .= 0
        push!(Hosts,Organism(tmp,[0 0 0 0 0 0 0 0 0 ; .5 .5 .5 .5 .5 .5 .5 .5 .5],4,4,i))
        if i <= NumPars
            partmp = zeros(4)
    ...
    while gen <= Generations
        ...
        for i in 1:NumHosts
            if i <= NumPars
                ...
                if Timing == "Synch"
                    ParIntro = rand(LifeSpan-Adol:LifeSpan-5)
                elseif Timing == "Asynch"
                    ParIntro = rand(2:LifeSpan - Adol)
                end
        ...
        for i in 1:NumHosts
            if i <= NumPars
                ...
                GenMatch = NSMatch(Hosts[i].Concentrations,Hosts[i].Net1_Size, Adol, LifeSpan)
...
function OrgLife(Host, Para, Adol, LifeSpan, InfTime, Credits)# function that carries out simulations of host life
    ...
    tmpConc = zeros(LifeSpan+5,NumProts) #+5 for 5 timesteps of 'burn-in' to allow the networks to reach Equilibrium
...
function NSMatch(x,Net1Size,Adol,LifeSpan)# function for determining how closely a host matches the dev input
    # The detector is the protein after the end of Net1, so Net1Size+1
    NSsignal = x[LifeSpan-Adol+1:end-3,Net1Size+1] 
    #the 3 TS offset accounts for the time it takes to stimulate the network leading to the increase of effector
    NSeff = x[LifeSpan+3-Adol+1:end,end-1]
...
function findPleioHosts(x,y,Runs)# function to find pleio hosts
    ...
    for i in 1:Runs
        #does not include pleiotropy at shared effector
        ImmToNS = count(y[i][1:x[i][1],x[i][1]+1:end-1] .!= 0)
        NSToImm = count(y[i][x[i][1]+1:end-1,1:x[i][1]] .!= 0)
...
for batch in 1:10
    ...
    for Run in 1:Runs #conducts Runs number of simulations
        #call simulation function
        print('\n',"******** TNOE ",Timing," Credit limit  Credit limit Variable Fitness Effects ", CredSwitch," ",OrgFitBalance," Batch ",batch," ********** ", (Dates.Time(Dates.now()) - Start_Time)/Nanosecond(1)* (1/1000000000))
        HostNetworks,NetSizes,HostFit[Run,:,:],ParNetworks,ParFit[Run,:,:],ParasiteBurden[Run,:],PerPopPleio[Run,:],tmpPleioFit,tmpNoPleioFit = Simulation(NumHosts,NumPars,Generations,OrgFitBalance,Timing,Start_Time,CredSwitch,Run);
    ...
    FileName = string(workDir,"/Data/Batches/",string(inf*100)[1:end-2],"_PercentInf_EC_Limit_Variable_",CredSwitch,"_FitEff_",OrgFitBalance,"_TNOE_",Timing,"_Batch_",batch,"_reverse.jld2")
