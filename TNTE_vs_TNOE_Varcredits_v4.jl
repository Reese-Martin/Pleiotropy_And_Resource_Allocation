# v2 has generations of evolutoin prior to competition
# v4 has added batch functionality to the code so that larger data sets can be generated
...
#region Funtions
function Simulation(NumHosts,NumPars,Generations,EQGens, OrgFitBalance,Timing,Start_Time,Run)# function that executes a single simulation consisting of "Generations" number of generations 
    ...
    while (gen <= Generations) & (Fixed == false)
        ...
        for i in 1:NumHosts
            if i <= NumPars
                ...
                HostFit[gen,i],Hosts[i].Par_Burden = OrgFitness(HostEffArea, OrgFitBalance, ParArea, GenMatch)
                if Hosts[i].Network_Type == "One Effector"
...
function culpoint(Host,a,NumHosts)
...
for batch in 1:10
    ...
    for Run in  1:Runs #conducts Runs number of simulations
        ...
        print('\n',"******** TNTE vs TNOE competition " ,Timing," Credit limit Variable Fitness Effects ",OrgFitBalance," EQGens ",EQGens," Batch ",batch," ********** ", (Dates.Time(Dates.now()) - Start_Time)/Nanosecond(1)* (1/1000000000))
        HostNetworks, NetSizes, HostFitTE[Run,:,:], HostFitOE[Run,:,:], ParNetworks, ParFit[Run,:,:], PerPopTE[Run,:] = Simulation(NumHosts,NumPars,Generations,EQGens,OrgFitBalance,Timing,Start_Time,Run);
    ...
    FileName = string(workDir,"/Data/Batches/",string(inf*100)[1:end-2],"_PercentInf_EC_Limit_Variable_FitEff_",OrgFitBalance,"_TNOE_vs_TNTE_",Timing,"_EQGens_",EQGens,"_Batch_",batch,".jld2")
