...
function OrgLife(Host, Para, Adol, LifeSpan, InfTime, Credits)# function that carries out simulations of host life
    ...
    for i in 1:LifeSpan+5-1 #-1 because we don't need to simulate the timestep after the end of the host lifespan
        if i in InfTime
...
for batch in 1:10
    ...
    for Run in 1:Runs #conducts Runs number of simulations
        ...
        print('\n',"******** TNTE " ,Timing," Credit limit Variable Fitness Effects ", CredSwitch," " ,OrgFitBalance," Batch ",batch," ********** ", (Dates.Time(Dates.now()) - Start_Time)/Nanosecond(1)* (1/1000000000))
    ...
    FileName = string(workDir,"/Data/Batches/",string(inf*100)[1:end-2],"_PercentInf_EC_Limit_Variable_",CredSwitch,"_FitEff_",OrgFitBalance,"_TNTE_",Timing,"_Batch_",batch,"_reverse.jld2")
